...
function perform_step!(
    ...
    compute_r̂ⁿ!(
        cache.r̂ⁿ, cache.X, state.r, state.z, cache.Ff₂, csts, cache.M_m₂xm₂_chol, cache.vec_m₂_1)

    update_state!(state, cache.X, cache.r̂ⁿ, τ)
...
function compute_Q!(
    ...
    #  Leading m₂×m₂ sub-block: Q ← 2M^{m₁×m₁} + cst1·K^{m₁×m₁} + cst2·M^{m₂×m₂}
...
+ \\frac{\\tau\\alpha}{q_5}
\\begin{bmatrix}
    M^{m_2 \\times m_2}\\bigl(2q_1 r^{n-1} - \\tau q_3 z^{n-1}\\bigr)
...
function compute_L!(
    ...
    @. cache.vec_m₂_2 = q₁2 * state.r - τq₃ * state.z

    # vec_m₂_1 ← M_m₂×m₂ · (2q₁rⁿ⁻¹ - τq₃zⁿ⁻¹)
...
= Q X
+ \\tau\\alpha G^{m_1}(X)
+ \\tau F^{m_1}\\bigl(\\frac{\\tau}{2}X + d^{n-1}\\bigr)
- L
= 0
```
for ``X = \\hat{v}^n`` via Newton's method, updating `cache.X` in-place. 
...
function newton_solve!(
    ...
    # Warm start: X ← vⁿ⁻¹
    cache.X .= state.v
    @. cache.d̂ⁿ = muladd(τ_2, cache.X, state.d)
...
+ L.
```
Assumes `cache.Q`, `cache.L`, `cache.X`, and cache.d̂ⁿ are already populated.
"""
function compute_minusH!(
    ...
    assembly_nonlinearity_G!(
        cache.vec_m₂_1, τα, input_data.g, cache.X, mesh1D, dof_map_m₂, quad)
...
JH 
= Q
+ \\tau\\alpha \\, JG\\bigl(\\hat{v}^n_{1:m₂}\\bigr)
...
Assumes `cache.Q`, and `cache.d̂ⁿ` are already populated.
"""
function compute_JH_upper!(
    ...
    JG = assembly_global_matrix_DG(
        τα, input_data.∂ₛg, cache.X, mesh1D, dof_map_m₂, quad)
    # FIXME: allocates
