...
# ============================================================================
"""
    example1_manufactured(a::Float64=4.4) -> PDEInputData
...
# Arguments
- `a::Float64=4.4`: Smoothness parameter controlling solution regularity.

# Returns
`PDEInputData` with analytical solution for convergence study.
"""
function example1_manufactured(a::Float64 = 4.4)
    ...
    # Analytical solutions
    u = (x, y, t) -> (x^a - x) * (y^a - 1.0) * (4.0 + t * t)
    v = (x, y, t) -> (x^a - x) * (y^a - 1.0) * (2.0 * t)
    ...
    ∂ₜₜu = (x, y, t) -> (x^a - x) * (y^a - 1.0) * 2.0
    Δu = (x, y,
        t) -> ((axa1 * x^a2) * (y^a - 1.0) +
               (x^a - x) * (axa1 * y^a2)) * (4.0 + t * t)
    ∂ₜₜz = function (x, t)
        xa_minus_x = x^a - x
        exp_term = 1.0 + exp(-x * x)
    ...
    f₂ = (
        x, t) -> q₁ * ∂ₜₜz(x, t) + q₂ * r(x, t) + q₃ * z(x, t) +
                 q₄ * v(x, ymin, t)

    # Initial conditions
    u₀ = (x, y) -> (x^a - x) * (y^a - 1.0) * 4.0
    ∂ₓu₀ = (x, y) -> (a * x^a1 - 1.0) * (y^a - 1.0) * 4.0
    ∂ᵧu₀ = (x, y) -> (x^a - x) * (a * y^a1) * 4.0
    v₀ = (x, y) -> 0.0
    ∂ₓv₀ = (x, y) -> 0.0
    ∂ᵧv₀ = (x, y) -> 0.0
    z₀ = x -> sinpi(x)
...
"""
    example1_zero_source(a::Float64=4.4) -> PDEInputData

Same configuration as `example1_manufactured` but with f₁ = f₂ = 0.
No analytical solution available.

# Arguments
- `a::Float64=4.4`: Smoothness parameter for initial conditions.

# Returns
`PDEInputData` with analytical solutions set to `nothing`.
"""
function example1_zero_source(a::Float64 = 4.4)
    # Precompute exponent-related constants
    a1 = a - 1.0
...
# ============================================================================
"""
    example2_manufactured(a::Float64=4.4) -> PDEInputData
...
`PDEInputData` with analytical solution for convergence study.
"""
function example2_manufactured(a::Float64 = 4.4)
    # Precompute exponent-related constants
    a1 = a - 1.0
    a2 = a - 2.0
    axa1 = a * a1
...
"""
    example2_zero_source(a::Float64=4.4) -> PDEInputData
...
`PDEInputData` with analytical solutions set to `nothing`.
"""
function example2_zero_source(a::Float64 = 4.4)
    # Precompute exponent-related constants
    a1 = a - 1.0
...
# ============================================================================
"""
    example3_manufactured(a::Float64=4.4) -> PDEInputData
...
`PDEInputData` with analytical solution for convergence study.
"""
function example3_manufactured(a::Float64 = 4.4)
    # Precompute exponent-related constants
    a1 = a - 1.0
    a2 = a - 2.0
    axa1 = a * a1
    ...
    z = (x, t) -> sinpi(x) - (1.0 + exp(-x * x)) * (x^a - x) * (t * t)
    r = (x, t) -> -(1.0 + exp(-x * x)) * (x^a - x) * (2.0 * t)
...
"""
    example3_zero_source(a::Float64=4.4) -> PDEInputData
...
`PDEInputData` with analytical solutions set to `nothing`.
"""
function example3_zero_source(a::Float64 = 4.4)
    # Precompute exponent-related constants
    a1 = a - 1.0
